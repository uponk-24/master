...
<div class="flex items-center justify-between mb-6">
    ...
    <a href="<?= site_url('admin/anggaran/tambah') ?>" class="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors">+ Tambah</a>
</div>
<?= alert_message() ?>
            ...
            <?php foreach ($budgets as $b): ?>
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-right">
                        <a href="<?= site_url('admin/anggaran/edit/' . $b['id']) ?>" class="text-primary hover:underline text-sm mr-3">Edit</a>
                        <form action="<?= site_url('admin/anggaran/hapus/' . $b['id']) ?>" method="POST" class="inline" onsubmit="return confirm('Hapus?')"><?= csrf_field() ?><button type="submit" class="text-red-500 hover:underline text-sm">Hapus</button></form>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
