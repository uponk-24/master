...
<div class="flex items-center justify-between mb-6">
    ...
    <a href="<?= site_url('admin/layanan/tambah') ?>" class="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors">+ Tambah</a>
</div>
<?= alert_message() ?>
            ...
            <?php foreach ($services as $i => $s): ?>
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-right">
                        <a href="<?= site_url('admin/layanan/edit/' . $s['id']) ?>" class="text-primary hover:underline text-sm mr-3">Edit</a>
                        <form action="<?= site_url('admin/layanan/hapus/' . $s['id']) ?>" method="POST" class="inline" onsubmit="return confirm('Hapus?')"><?= csrf_field() ?><button type="submit" class="text-red-500 hover:underline text-sm">Hapus</button></form>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
